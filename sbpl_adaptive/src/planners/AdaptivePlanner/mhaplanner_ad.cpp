    ...
    int* solcost)
{
    ...
    int succ = replan(solution_stateIDs_V, params, solcost);

    if(succ) {
        write_sol_to_file(*solution_stateIDs_V);
    }
...
long int MHAPlanner_AD::compute_key(MHASearchState* state, int hidx, int w)
{
    return (long int)state->g + (long int)(w * m_eps * (long int)state->od[hidx].h);
}
    ...
    for (size_t sidx = 0; sidx < succ_ids.size(); ++sidx)  {
        ...
        if (new_g < succ_state->g) {
            ...
            if (!closed_in_anc_search(succ_state)) {
                long int fanchor = compute_key(succ_state, 0);
...
void MHAPlanner_AD::write_sol_to_file( const std::vector<int>& sol) {

    std::string file_name = "/home/karthik/humanoid_path.csv";

    for(int sid : sol)
        space_->writeStateDataToFile(sid, file_name);
}
